...
def model_predictions(test_data, labels, MODEL_DIR):
...
MODEL_DIR = f'/media/jonas/SSD_new/CMS/Semester_4/research_project/history/age/linea_all_data/history/Model_age_regression_linear/kernel_6/pooling_max_pool/learning_rate_0.001/fold_1/model'
model = tf.keras.models.load_model(MODEL_DIR)

model.evaluate(test_data)


predictions = model_predictions(test_data_inference, labels, MODEL_DIR)
...
predictions.columns = ['pred', 'true']
print(predictions.head())

sns.set_style("darkgrid")

plt.figure()
sns.regplot(x='true', y='pred', data=predictions, ci=None, line_kws={'color':'red'}, scatter_kws={'edgecolors':'white', 'alpha':1, 'linewidths':0.5})
sns.lineplot(data=perfect_df, x='pred', y='true', color='black', linestyle='--')
plt.xlim((-5, 100))
plt.ylim((-5, 100))
plt.xlabel('age true [years]')
plt.ylabel('age predicted [years]')
#plt.title('Fold 1')
plt.legend(['Individual Predictions','Linear Regression Line', 'Optimal Prediction Line'], loc='upper left')
plt.tight_layout()
