...
for learning_rate_choice in LEARNING_RATE:
    ...
    for train_index, test_index in kfold.split(patient_number_array_unique):
        ...
        base_output_1 = model.get_layer('global_average_pooling1d_frozen').output
        base_output_2 = model.get_layer('global_average_pooling1d_1_frozen').output


        x_1 = tf.keras.layers.Dense(20, activation='relu')(base_output_1)
        x_1 = tf.keras.layers.Dense(1, activation='linear')(x_1)

        x_2 = tf.keras.layers.Dense(20, activation='relu')(base_output_2)
        ...
        model = tf.keras.Model(inputs=model.input, outputs=output_layer, name='Model_3_transfer_learning_3')
