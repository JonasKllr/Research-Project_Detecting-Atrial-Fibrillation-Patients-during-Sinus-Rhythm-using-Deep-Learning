...
for learning_rate_choice in LEARNING_RATE:
    ...
    for train_index, test_index in kfold.split(patient_number_array_unique):
        ...
        base_output_1 = model.get_layer('batch_normalization_2_frozen').output
        base_output_2 = model.get_layer('batch_normalization_6_frozen').output



        x_1 = tf.keras.layers.Conv1D(filters=20, kernel_size=5, strides=1, padding='same', activation='relu')(base_output_1)
        ...
        model = tf.keras.Model(inputs=model.input, outputs=output_layer, name='Model_3_transfer_learning_4')
