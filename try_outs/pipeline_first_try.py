...
signals = np.empty((200, 1280, 2, 1)) # TODO make it dynamic depending on number of records (record_count)
...
for filename in sorted(os.listdir(directory)):
    ...
    if filename.endswith('.dat'):
        ...
        if (os.path.basename(filename)[0] == 'p'):
            ...
            signals[record_count, :, :, 0] = wfdb.rdsamp(file_directory, channels=[0, 1], sampto=1280)[0]
...
input_layer = tf.keras.Input(shape=(RECORD_LENGTH, 2, 1))

# TODO change to conv1D
x = tf.keras.layers.Conv2D(filters=32, kernel_size=(10, 1), strides=(3, 1), activation='relu')(input_layer)
x = tf.keras.layers.Conv2D(filters=32, kernel_size=(10, 1), strides=(3, 1), activation='relu')(x)
x = tf.keras.layers.Conv2D(filters=32, kernel_size=(10, 1), strides=(3, 1), activation='relu')(x)
x = tf.keras.layers.Conv2D(filters=32, kernel_size=(10, 1), strides=(3, 1), activation='relu')(x)
