...
for learning_rate_choice in LEARNING_RATE:
    ...
    for train_index, test_index in kfold.split(patient_number_array_unique):
        ...
        for layer in model.layers:
            layer._name = layer.name + str("_trained")

        model.summary()
        
        base_output_1 = model.get_layer('dropout_1_trained').output
        base_output_2 = model.get_layer('dropout_4_trained').output

        x_1 = tf.keras.layers.Conv1D(filters=40, kernel_size=5, strides=2, padding='same', activation='relu')(base_output_1)
        ...
        x_2 = tf.keras.layers.Conv1D(filters=40, kernel_size=5, strides=2, padding='same', activation='relu')(base_output_2)
        ...
        model = tf.keras.Model(inputs=model.input, outputs=output_layer, name='Model_2_transfer_learning')
